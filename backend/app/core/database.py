...
from app.models.interview_feedback import InterviewFeedback, InterviewTimestampFeedback
...
async def connect_db():
    ...
    # Interview Timestamp Feedback Indexes
    await InterviewTimestampFeedback.get_motor_collection().create_index("feedback_id", unique=True)
    await InterviewTimestampFeedback.get_motor_collection().create_index([("interview_id", 1), ("timestamp_seconds", 1)])
    await InterviewTimestampFeedback.get_motor_collection().create_index([("student_id", 1), ("timestamp_seconds", 1)])
